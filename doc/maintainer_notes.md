...
Each cluster in a platform instance has a dedicated vpc. Naming convention assumes vpc's are named "cluster name"-vpc. This enables later fetching of vpc info just by knowning the cluster instance name.  
...
* Local terraform, python, and ruby version config files are ignored.
...
2. Pre-commit checks. In addition to typical linting and style checks, several terraform code checks are run. This aligns with static checks performed in the pipeline.
...
* Circleci Executor: [ghcr.io/twplatformlabs/circleci-infra-aws](https://github.com/twplatformlabs/circleci-infra-aws)
...
* TW organization has turned guardduty, however permissions are blocked for individual account configurations or discovery of the settings. Not running the guardduty agent on EKS.